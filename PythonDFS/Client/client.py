...
def authenticate():
	...
	for i in range(0, 4):
		if auth_status == 'Valid password.':
			...
			print("password after md5: ",password)
	...
	print("final_authorization: ",final_authorization)
...
def client():
	...
	try:
		...
		status1 = ('Connected to server', 'Server1')
		print(status1[0], status1[1])
		time.sleep(1)
	except ConnectionRefusedError:
		status1 = ('Could not connect to server', 'Server1')
		print(status1[0], status1[1])
	...
	try:
		...
		status2 = ('Connected to server', 'Server2')
		print(status2[0], status2[1])
		time.sleep(1)
	except ConnectionRefusedError:
		status2 = ('Could not connect to server', 'Server2')
		print(status2[0], status2[1])
	...
	try:
		...
		status3 = ('Connected to server', 'Server3')
		print(status3[0], status3[1])
		time.sleep(1)
	except ConnectionRefusedError:
		status3 = ('Could not connect to server', 'Server3')
		print(status3[0], status3[1])
	...
	try:
		...
		status4 = ('Connected to server', 'Server4')
		print(status4[0], status4[1])
		time.sleep(1)
	except ConnectionRefusedError:
		status4 = ('Could not connect to server', 'Server4')
		print(status4[0], status4[1])	
	...
	for i in range(0,4):
		try:
			...
			print('From ' +SERVERS[i] +': ' +response.decode())
		except OSError:
			pass
	...
	if command.lower() == 'put':
		...
		server1, server2, server3, server4 = chunk_pairs(filename)
		...
		for i in range(0,4):
			try:
				conns[i].send(servers[i][0].encode())
				time.sleep(0.5)
				chunk1=open(servers[i][0], 'rb').read()
				conns[i].send(chunk1)
				print('\nSending ' +str(servers[i][0]) +'...\n')
			except OSError:
				pass
		...
		for i in range(0,4):
			try:
				...
				if response == 'Chunk 1 successfully transferred.\n':
					print(SERVERS[i] +' Chunk 1 transfer complete.')
				else:
					print(SERVERS[i] +' Chunk 1 transfer failed.')
			except OSError:
				pass
		...
		for i in range(0,4):
			try:
				conns[i].send(servers[i][1].encode())
				time.sleep(0.5)				
				chunk2=open(servers[i][1], 'rb').read()
				conns[i].send(chunk2)
				print('\nSending ' +str(servers[i][1]) +'...\n')
			except OSError:
				pass
		...
		for i in range(0,4):
			try:
				...
				if response == 'Chunk 2 successfully transferred.\n':
					print(SERVERS[i] +' Chunk 2 transfer complete.')
				else:
					print(SERVERS[i] +' Chunk 2 transfer incomplete.')
			except OSError:
				pass
			
		# delete chunks from client directory
		os.remove(str(server1[0]))
		os.remove(str(server1[1]))
		os.remove(str(server3[0]))
		os.remove(str(server3[1]))
	...
	# LIST
		...
		for i in range(0,4):		
			try:
				...
				print('\nCurrent ' +SERVERS[i] +'\%s files:' %username)
				print('-' * 27)
				print(file_names)
			except OSError:
				pass
