...
def auth_params():
	...
	config_file='C:\distributed_file_system\PythonDFS\server_conf.conf'
	...
	# auth_dict is global and can be accessed everywhere
	global auth_dict 
	auth_dict = {}
	for i in range(0, len(users)):
...
def client_auth(auth_dict, username, password):
	ct = 0
	auth_status=''
	print("len of auth_dict: ",len(auth_dict))
			...
			if ct < len(auth_dict):
				...
				if username == key:
					...
					if password == value:
						...
						auth_status='Authorization has been Granted.\n'
						print(auth_status)
						conn.send(auth_status.encode())
						pass
					else:
						print('Incorrect password.')
						auth_status = 'Authorization has been Denied.\n'
						print(auth_status)
						conn.send(auth_status.encode())
						sys.exit()
				else:
...
def put(new_dir_path):
	...
	try:
		...
		print('Incorrect Buffer Size.The buffer size is not a number. \nExiting now...')
		sys.exit()
...
def get(username):
	...
	print('User ' +username +' has requested: ' +filename)
	...
	print("user_dir_filelist",user_dir_filelist)
